...
const Navbar = ({ logo, children, active, onCallback, className }: NavbarProps) => {
  return (
    <NavbarContext.Provider value={{ active, onCallback }}>
      <div className={`w-full flex justify-between items-center h-[82px] text-lg ${className}`}>
        <div>{logo}</div>
        <div className="hidden lg:flex gap-8 items-center pr-4">{children}</div>
        <div className="flex lg:hidden gap-8 items-center pr-4">
          <Dropdown openOnClick closeOnLeave={false} className="min-w-[400px] pr-[100px]" buttonProps={{iconProps: "menu-outline"}} >
            {children}
          </Dropdown>
        </div>
      </div>
